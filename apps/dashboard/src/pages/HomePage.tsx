...
export function HomePage() { const { user } = useAuth(); return <div className="mx-auto max-w-7xl"><div className="mb-9"><p className="text-sm font-medium text-brand-600">Welcome back, {user?.firstName}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900">Where do you want to look?</h2><p className="mt-3 max-w-2xl text-base leading-7 text-slate-500">Choose an operational workspace to begin. This dashboard is read-only, so your focus stays on visibility and decision-making.</p></div><div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => <NavigationCard key={card.to} {...card}/>)}</div></div>; }
