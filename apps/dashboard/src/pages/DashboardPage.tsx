  ...
  return <div className="mx-auto max-w-7xl space-y-8">
    <div>
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3 rounded-xl border bg-white p-4 shadow-sm">
        ...
        <label className="text-xs font-medium text-slate-500">Terminal<select aria-label="Terminal" value={filters.terminalId} onChange={setFilter('terminalId')} className="mt-1 block h-10 min-w-44 rounded-lg border bg-white px-3 text-sm text-slate-700"><option value="">All terminals</option>{terminals.data?.map(terminal => <option key={terminal.id} value={terminal.id}>{terminal.terminalCode} — {terminal.city}</option>)}</select></label>
