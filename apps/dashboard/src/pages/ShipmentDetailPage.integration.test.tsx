...
describe('ShipmentDetailPage integration', () => {
  beforeEach(() => {
    vi.mocked(shipmentApi.detail).mockResolvedValue({ id: 'shipment-1', shipmentNumber: 'SHIP-100', referenceNumber: 'ORDER-100', status: 'PARTIALLY_DELIVERED', originTerminalId: 1, destinationTerminalId: 2, transitDays: 2, estimatedDeliveryAt: '2026-07-15T12:00:00Z', createdAt: '', updatedAt: '', originTerminal: terminal(1, 'YYC'), destinationTerminal: terminal(2, 'YEG'), packages: [], snapshot: { currentStatus: 'PARTIALLY_DELIVERED', currentTerminalId: 2, packageCount: 2, deliveredPackages: 1, outForDeliveryPackages: 0, remainingPackages: 1, progressPercent: 50, completedAt: null, lastActivityAt: '2026-07-13T12:00:00Z' } });
    vi.mocked(shipmentApi.packages).mockResolvedValue([{ id: 'package-1', trackingNumber: 'PKG-100', packageType: 'CONVEYABLE', currentStatus: 'DELIVERED', currentTerminalId: 2, currentContainerId: null, currentTrailerId: null, currentRouteId: null, currentTruckId: null, updatedAt: '2026-07-13T12:00:00Z' }]);
    vi.mocked(shipmentApi.history).mockResolvedValue([{ id: 'event-1', shipmentId: 'shipment-1', sourcePackageEventId: null, eventType: 'PACKAGE_ASSIGNED', correlationId: 'request-1', createdAt: '2026-07-13T11:00:00Z' }]);
  });
