...
export function ShipmentDetailPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric icon={PackageOpen} label="Packages" value={snapshot?.packageCount ?? packages.length}/><Metric icon={CheckCircle2} label="Delivered" value={snapshot?.deliveredPackages ?? 0}/><Metric icon={PackageCheck} label="Remaining" value={snapshot?.remainingPackages ?? packages.length}/><DateMetric label="Estimated delivery" value={shipment.estimatedDeliveryAt ? timestamp(shipment.estimatedDeliveryAt) : 'Not committed'} detail={shipment.transitDays ? `${shipment.transitDays} transit day${shipment.transitDays === 1 ? '' : 's'}` : 'Legacy shipment'}/></div>
...
function DateMetric({ label, value, detail }: { label: string; value: string; detail: string }) { return <section className="rounded-2xl border bg-white p-5 shadow-sm"><p className="text-xs font-medium uppercase tracking-wide text-brand-700">{label}</p><p className="mt-4 text-base font-semibold text-slate-900">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></section>; }
