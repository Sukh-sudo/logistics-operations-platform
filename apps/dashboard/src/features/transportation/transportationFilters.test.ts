...
describe('transportation filters', () => {
  it('filters terminal records by an inclusive creation date range', () => {
    const terminals = [terminal(1), terminal(2, '2026-07-02T10:00:00.000Z'), terminal(3, '2026-07-03T10:00:00.000Z')];
    expect(filterTerminals(terminals, { ...emptyTransportationFilters, fromDate: '2026-07-01', toDate: '2026-07-02' }).map(item => item.id)).toEqual([1, 2]);
  });
  it('combines date, origin, and destination for routes', () => {
    ...
    expect(filterRoutes(routes, { fromDate: '2026-07-01', toDate: '2026-07-02', originId: '1', destinationId: '2' }).map(item => item.id)).toEqual(['match']);
  });
  it('uses planned departure as the trip date and route terminals as its lane', () => {
    ...
    expect(filterTrips(trips, { fromDate: '2026-07-03', toDate: '2026-07-03', originId: '1', destinationId: '2' }).map(item => item.id)).toEqual(['match']);
  });
  it('keeps all shipments when filters are clear and narrows them when combined', () => {
    ...
    expect(filterShipments(shipments, { fromDate: '', toDate: '2026-07-02', originId: '1', destinationId: '2' }).map(item => item.id)).toEqual(['match']);
  });
});
