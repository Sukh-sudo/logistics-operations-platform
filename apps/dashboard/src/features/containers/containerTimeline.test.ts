...
describe('toContainerTimelineItem', () => {
  it('maps a container event into a readable timeline entry', () => {
    const event: ContainerEventDto = { id: 'event-1', containerId: 'container-1', eventType: 'CONTAINER_CLOSED', employeeId: 42, correlationId: 'request-1', metadata: null, createdAt: '2026-07-08T14:00:00Z' };

    expect(toContainerTimelineItem(event)).toEqual({
      id: 'event-1',
      title: 'CONTAINER CLOSED',
      occurredAt: '2026-07-08T14:00:00Z',
      details: ['Employee 42', 'Correlation request-1'],
    });
  });
});
